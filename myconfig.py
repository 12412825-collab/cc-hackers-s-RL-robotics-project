...
# ============================================================================
# Residual RL Configuration (SAC — Soft Actor-Critic)
# ============================================================================
# Enable Residual RL module. When True, a ResidualPilot Part is added to the
# Vehicle pipeline alongside the base KerasPilot. The final steering is:
#   steering = base_pilot.angle + residual.steering * RESIDUAL_SCALE
RESIDUAL_RL = False

# Scale factor for residual steering output.
# Residual network outputs in [-1, 1]; this scales it to [-0.3, 0.3].
# Higher values give RL more authority but may cause instability.
RESIDUAL_SCALE = 0.3

# Path to saved Residual RL model weights (.pth file)
# Set this to load a pre-trained model for inference.
...
# Discount factor (gamma) for RL
RESIDUAL_GAMMA = 0.99

# Soft update coefficient for target networks (tau)
RESIDUAL_TAU = 0.005
...
# --- Backbone Architecture (Transfer Learning) ---
# Visual encoder backbone for the Residual RL model:
#   'mobilenet_v3_small' — MobileNetV3-Small (★ default, ~2.5M params, best for embedded)
#   'mobilenet_v2'       — MobileNetV2 (~3.5M params, good balance)
#   'efficientnet_b0'    — EfficientNet-B0 (~5.3M params, highest accuracy)
#   'donkey_cnn'         — Original 5-conv CNN from scratch (baseline, no pretrain)
#
# Pretrained backbones use ImageNet weights. The backbone is frozen by default
# (feature extractor mode); only the projection head + SAC heads are trained.
RESIDUAL_BACKBONE = 'mobilenet_v3_small'

# Output dimension of the visual encoder (input to SAC Actor/Critic heads).
# DonkeyCNN uses 50 by default. Pretrained backbones use a projection head
# to map from backbone_dim (e.g. 576 for MobileNetV3-Small) to this dimension.
RESIDUAL_FEATURE_DIM = 50

# Freeze pretrained backbone weights during initial training.
# True  = transfer learning: backbone is frozen, only train projection + SAC heads
# False = full fine-tuning: all weights updated (use with lower LR, more data)
RESIDUAL_FREEZE_BACKBONE = True

# Logging frequency (in frames) for ResidualPilot debug output
RESIDUAL_LOG_FREQ = 200

# ============================================================================
# Multi-Modal Sensor Fusion Configuration
# ============================================================================
# Enable multi-modal input for Residual RL (image + sensor observation).
# When True, the SAC agent fuses visual features from the camera with
# a multi-sensor observation vector from Encoder/IMU/HC-SR04 Ultrasonic.
# Requires parts/sensors.py for the SensorFusion module.
# When False, the SAC agent operates in image-only mode (backward compatible).
USE_MULTI_MODAL = False

# Dimension of the sensor observation vector produced by SensorFusion.
# Layout (9-dim):
...
# Output dimension of the SensorEncoder MLP (fused with visual features).
# Visual features (RESIDUAL_FEATURE_DIM: 50) + Sensor features (32) = 82 fused.
SENSOR_FEATURE_DIM = 32

# Hidden dimension of the SensorEncoder MLP
SENSOR_HIDDEN_DIM = 64

# ============================================================================
# Sensor Enable Flags (individual sensors in SensorFusion)
# ============================================================================
# Wheel Encoder / Odometry — provides speed + acceleration
HAVE_ODOM = False
ENABLE_ENCODER = False        # Enable encoder input to SensorFusion
ENCODER_TYPE = 'GPIO'         # 'GPIO' | 'Arduino' | 'Simulated'
ENCODER_MM_PER_TICK = 12.7625 # mm travel per encoder tick
ENCODER_MAX_SPEED = 5.0       # m/s, for normalization

# IMU (Inertial Measurement Unit) — 6-DOF acceleration + gyroscope
HAVE_IMU = False
ENABLE_IMU = False            # Enable IMU input to SensorFusion
IMU_SENSOR = 'mpu6050'        # 'mpu6050' | 'mpu9250' | 'icm20948'
IMU_ACCEL_RANGE = 2.0         # ±g
IMU_GYRO_RANGE = 250.0        # ±deg/s

# HC-SR04 Ultrasonic Distance Sensor — front obstacle detection
ENABLE_OBSTACLE = False       # Enable HC-SR04 input to SensorFusion
OBSTACLE_TYPE = 'HC_SR04'     # 'HC_SR04' | 'tfmini' | 'infrared'
OBSTACLE_MAX_DIST = 400.0     # cm, for normalization
OBSTACLE_MIN_DIST = 2.0       # cm, minimum detectable distance

# Sensor Fusion Parameters
SENSOR_FUSION_RATE = 20.0     # Hz, sensor fusion output rate
