...
# Residual RL — conditionally imported when RESIDUAL_RL=True in myconfig.py
try:
    from parts.residual_rl import ResidualPilot
    RESIDUAL_RL_AVAILABLE = True
except ImportError:
    RESIDUAL_RL_AVAILABLE = False
...
def drive(cfg, model_path=None, use_joystick=False, model_type=None,
    ...
    if model_path:
        ...
        #
        # Residual RL Pilot — runs in parallel with the base pilot.
        # Reads the same (possibly transformed) camera image and outputs
        # a residual steering correction that is combined with the base
        # pilot's steering by ResidualDriveMode.
        #
        if getattr(cfg, 'RESIDUAL_RL', False):
            if not RESIDUAL_RL_AVAILABLE:
                raise ImportError(
                    "RESIDUAL_RL=True but parts/residual_rl.py not found. "
                    "Ensure parts/residual_rl.py exists and PyTorch is installed."
                )
...
class ResidualDriveMode:
    """
    Extended DriveMode that incorporates residual RL steering correction.

    When RESIDUAL_RL is enabled, this replaces DriveMode.
    The final steering in autopilot modes is:
        steering = base_pilot_steering + residual_steering
    where residual_steering is the output of ResidualPilot (already scaled).
    """
    def __init__(self, ai_throttle_mult=1.0):
        self.ai_throttle_mult = ai_throttle_mult
    ...
    def run(self, mode,
        ...
        if mode == 'user':
            ...
            base = pilot_steering if pilot_steering else 0.0
            resid = residual_steering if residual_steering else 0.0
            return base + resid, user_throttle
        base = pilot_steering if pilot_steering else 0.0
        resid = residual_steering if residual_steering else 0.0
        return (base + resid,
                pilot_throttle * self.ai_throttle_mult if pilot_throttle else 0.0)
